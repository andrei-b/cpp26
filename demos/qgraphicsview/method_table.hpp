...
template <typename T>
inline constexpr bool is_effectively_complete_v =
    std::is_void_v<std::remove_cvref_t<T>> || is_complete_type<std::remove_cvref_t<T>>::value;

template <class Tuple>
struct tuple_all_effectively_complete;

template <typename... Ts>
struct tuple_all_effectively_complete<std::tuple<Ts...>>
    : std::bool_constant<(is_effectively_complete_v<Ts> && ...)> {};
...
auto get_method_table() {
    ...
    template for (constexpr auto m : std::define_static_array(std::meta::members_of(^^C, ctx))) {
        if constexpr (std::meta::is_function(m) && std::meta::has_identifier(m)) {
            ...
            if constexpr (accessible_from_current) {
                auto pmf = &[:m:];
                using PMF = decltype(pmf);
                using traits = function_traits<PMF>;
                constexpr bool has_complete_invoke_types =
                    is_effectively_complete_v<typename traits::return_type> &&
                    tuple_all_effectively_complete<typename traits::args_tuple>::value;
                constexpr auto argcount = get_parameter_count<m>();
                auto return_type = type_name<PMF>();

                if constexpr (has_complete_invoke_types) {
